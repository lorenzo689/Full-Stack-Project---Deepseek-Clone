...
const Sidebar: React.FC<SidebarProps> = ({expand, setExpand}: SidebarProps) => {

  const {openSignIn} = useClerk()
  const {user} = useAppContext()
  const [openMenu, setOpenMenu] = useState<OpenMenuState>({id: 0, open: false})
  ...
  return (
      ...
      <div>
        <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
            ...
            className='group relative flex items-center justify-center hover:bg-gray-500/20 transition-all duration-300 h-9 w-9 aspect-square rounded-lg cursor-pointer'>
                ...
                <Image src={expand ? assets.sidebar_close_icon : assets.sidebar_icon} alt='Sidebar' className='hidden md:block w-7'/>
        ...
        <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? "bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max" : "group relative h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
          <Image className={expand ? 'w-6' : 'w-7'} src={expand ? assets.chat_icon : assets.chat_icon_dull} alt='ChatIcon'/>
    ...
    <div>
      <div className={`flex items-center cursor-pointer group relative ${expand ? "gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
          <Image className={expand ? "w-5" : "w-7 mx-auto"} src={expand ? assets.phone_icon : assets.phone_icon_dull} alt='PhoneIcon'/>
     ...
     <div onClick={() => {if (!user) openSignIn();}}
